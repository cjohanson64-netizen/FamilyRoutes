...
import type { ActionPipelineStepNode, ProgramNode } from "./ast/nodeTypes";
import {
  applyRuntimeAction,
...
} from "./runtime/relationshipComparison";
...
export function prepareTatMutationTransaction(
  ...
  const materializedActions = request.actions.map((action) => {
    ...
    const generatedTarget = findGeneratedNodeIdPrefix(runtimeAction.pipeline);
    if (!generatedTarget) {
      return action;
    }
    ...
    return {
      ...
      payload: {
        ...action.payload,
        target: generateRuntimeNodeId(graph, generatedTarget),
      },
    };
  });
...
function applyRuntimeMutationAction(
  ...
  switch (action.type) {
    ...
    case "action": {
      applyRuntimeActionToGraph(graph, state, action.payload, { causedBy });
      return;
    }
